...
char *str;
{
  ...
  sprintf(scratch,"Wildcard for aliases and triggers set to '%c'\n",wildcard);
  shell_print(sh,scratch);
}
...
char *str;
{
  ...
  if (b>0) bailout=b;
  else shell_print("Argument is a number.");
}
...
int count;
{
  ...
  if (sh->flags&COMMAND) command_processing(sh,sh->key_buff);
