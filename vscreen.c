...
void null(),back_char(),forward_char(),newline(),
        toggle_command(),break_out(),delete(),backspace(),ccomplete(),
        create_shell(),quit_shell(),go_next_shell(),go_previous_shell(),
        del_line(),go_start_line(),transpose(),go_end_line(),repaint(),
        toggle_secrecy();

extern void next_history(),prev_history();
...
void handle_user()
{
  ...
  if (ioctl(0,FIONREAD,&chars_ready) == -1) handle_error("Ioctl Failed");
